...
import static edu.ace.infinite.utils.videoCache.Preconditions.checkNotNull;
...
import edu.ace.infinite.utils.videoCache.file.DefaultFileNameGenerator;
...
public class HttpProxyCacheServer {
     ...
     * <p>
     * Calling this method has same effect as calling {@link #getProxyUrl(String, boolean,String)} with 2nd parameter set to {@code true}.
     *
     ...
     */
    public String ID;
    public String getProxyUrl(String url,String ID) {
        this.ID = ID;
        return getProxyUrl(url, true,ID);
    }
     ...
     */
    public String getProxyUrl(String url,boolean allowCachedFileUri, String ID) {
        if (allowCachedFileUri && isCached(ID)) {
    ...
    private File getCacheFile(String ID) {
        ...
        String fileName = config.fileNameGenerator.generate(ID);
    ...
    private void processSocket(Socket socket) {
        try {
            ...
            if (pinger.isPingRequest(url)) {
                ...
                HttpProxyCacheServerClients clients = getClients(url,ID);
    ...
    private HttpProxyCacheServerClients getClients(String url,String ID) throws ProxyCacheException {
        synchronized (clientsLock) {
            HttpProxyCacheServerClients clients = clientsMap.get(ID);
            if (clients == null) {
                clients = new HttpProxyCacheServerClients(config,url,ID);
    ...
    public static final class Builder {
        ...
        private static final long DEFAULT_MAX_SIZE = 500 * 1024 * 1024;
        ...
        /**
         * Overrides default cache file name generator {@link DefaultFileNameGenerator} .
         *
